...
class SelfieValidationResult:
    ...
    def format_korean(self) -> str:
        """검수표 템플릿 형식으로 출력 (skills/fnf-image-gen/셀카_selfie/검수표_템플릿.md 참조)"""
        ...
        lines = [
            ...
            "| 기준 | 비중 | Pass 조건 | 점수 | 통과 |",
            "|------|------|-----------|------|------|",
            f"| 리얼리즘 | 35% | >= 80 | {self.realism} | {check(self.realism, 80)} |",
            f"| 인물 보존 | 25% | >= 80 | {self.person_preservation} | {check(self.person_preservation, 80)} |",
            f"| 시나리오 적합성 | 20% | >= 70 | {self.scenario_fit} | {check(self.scenario_fit, 70)} |",
            f"| 피부 상태 | 10% | >= 70 | {self.skin_condition} | {check(self.skin_condition, 70)} |",
            f"| 역검증 | 10% | >= 60 | {self.anti_polish_factor} | {check(self.anti_polish_factor, 60)} |",
            "",
            f"**총점**: {self.total_score}/100 | **등급**: {self.grade} | **판정**: {'PASS' if self.passed else 'FAIL'}",
        ]

        if self.too_polished_penalty:
            lines.append("")
            lines.append("**주의**: 너무 완벽함 - 자연스러움 부족")
