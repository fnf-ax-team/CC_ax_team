...
├── analyzer.py           # VLM 분석 (착장)
├── prompt_builder_v2.py  # 프롬프트 조립 v2 (한국어 스키마)
├── generator_v2.py       # 이미지 생성 v2 (배경 레퍼런스 지원)
├── retry_generator_v2.py # 검증+재시도 v2 (배경 레퍼런스 지원)
├── edit_generator.py     # 편집 모드 (edit_brandcut, edit_with_validation)
├── validator_v2.py       # 14개 기준 검증 v2
...
```python
from core.ai_influencer.presets import (
    load_mlb_preset,              # 프리셋 로드
    load_mlb_preset_with_image,   # 이미지 포함 로드
    list_mlb_presets,             # 프리셋 목록
    get_random_expression,        # 랜덤 표정
    mlb_pose_preset_to_analysis,  # 포즈→PoseAnalysisResult 변환
)

# 포즈 프리셋 로드
pose = load_mlb_preset("pose", "MLB스탠딩_주머니_01")

# 포즈 프리셋 → PoseAnalysisResult 변환 (프롬프트 빌더 연동)
from core.ai_influencer.presets import mlb_pose_preset_to_analysis
pose_analysis = mlb_pose_preset_to_analysis(pose)

# 배경 프리셋 (이미지 포함)
bg_preset, bg_image = load_mlb_preset_with_image("background", "MLB스튜디오_그레이_01")
...
```python
from core.ai_influencer.presets import (
    load_mlb_preset, load_mlb_preset_with_image,
    mlb_pose_preset_to_analysis, get_random_expression,
)
from core.brandcut import build_prompt, generate_with_validation
...
# 2. 프롬프트 조립
prompt = build_prompt(
    outfit_analysis=outfit_result,
    pose_analysis=pose_analysis,      # PoseAnalysisResult
    expression_preset=expression,      # 표정 프리셋
)
...
result = generate_with_validation(
    ...
    background_reference=bg_image,     # NEW: 배경 레퍼런스 이미지
)
```
...
images = generate_brandcut(
    ...
    temperature=0.7,
    pose_reference=pose_ref_image,
    expression_reference=expr_ref_image,    # 표정 레퍼런스 (선택)
    background_reference=bg_ref_image,      # NEW: 배경 레퍼런스 (선택)
)
...
**버전**: 3.3.0 (MLB 프리셋 연동 + 배경 레퍼런스)
**작성일**: 2026-03-09

**변경사항 (v3.3.0)**:
- MLB 스타일 프리셋 DB 연동 (`db/presets/brandcut/mlb/` 9개 JSON)
- `mlb_pose_preset_to_analysis()` 추가 - 포즈 프리셋 → PoseAnalysisResult 변환
- `background_reference` 파라미터 추가 (generator_v2.py, retry_generator_v2.py)
- 배경 레퍼런스 이미지를 API에 직접 전송 지원
- prompt_builder_v2.py에 PoseAnalysisResult 타입 체크 추가
- 모듈 구조 v2 파일 기준으로 문서 업데이트
