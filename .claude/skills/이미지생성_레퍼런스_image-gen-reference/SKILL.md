...
description: 이미지 생성 워크플로 구축 시 참조하는 코드 패턴, 프롬프트 템플릿, 유틸리티 함수 모음. 모델/해상도/Temperature/API키 등 절대 규칙은 CLAUDE.md 참조.
---

# 이미지 생성 레퍼런스 (Image Generation Reference)

> **절대 규칙 (모델, 해상도, Temperature, API키, 에러 처리)은 `CLAUDE.md`에 정의되어 있습니다.**
> 이 스킬은 코드 패턴, 프롬프트 템플릿, 유틸리티 함수 등 **구현 시 참조하는 레퍼런스**입니다.
>
> 워크플로별 스킬:
...
> 키 로테이션 규칙은 `CLAUDE.md` 참조. 아래는 구현 코드 레퍼런스.
...
> 에러 코드 및 재시도 정책은 `CLAUDE.md` 참조. 아래는 구현 코드 레퍼런스.
...
| 스킬 | 용도 | 이 레퍼런스 활용 부분 |
|------|------|---------------------|
| `브랜드컷_brand-cut` | 브랜드 패션 화보 | 전체 (코드 패턴 + 프롬프트 + 배경) |
| `배경교체_background-swap` | 배경 교체 | Section 2-3, 7-8 |
| (향후 추가 워크플로) | - | Section 2 (최소) |
