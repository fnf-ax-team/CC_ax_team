...
얼굴 동일성 35% + 포즈 정확도 20% 비중으로 강화 검증
- 셀카 스킬(25%)보다 높은 얼굴 비중
- 포즈 레퍼런스 비교 추가 (step-by-step 강제)
- 동일 캐릭터 유지 + 포즈 정확도가 핵심
"""
...
### 2. pose_accuracy (포즈 정확도) - 20%
[POSE REFERENCE]와 생성 이미지의 포즈를 비교합니다.

[STEP 1] POSE REFERENCE 분석:
- 프레이밍(어디서 끊기는가) = ? (전신/상반신/허벅지에서 끊김 등)
- 카메라 앵글 = ? (아이레벨/로앵글/하이앵글)
- stance = ? (서있기/앉기/기대기 등)
- 왼팔 위치 = ?
- 오른팔 위치 = ?
- 다리 형태 = ?
...
- 카메라 앵글: 같음(0) / 다름(-15)
- stance: 같음(0) / 다름(-20)
- 팔 위치: 같음(0) / 한쪽 다름(-10) / 양쪽 다름(-20)
- 다리 형태: 같음(0) / 다름(-15)

최종 점수 = 100 - 합계 감점 (최소 0)
reason 필수 형식: "REF:전신+로앵글+서있기, GEN:상반신+아이레벨+서있기, 감점:-35"
...
```json
{
    ...
    "pose_accuracy": {"score": 0-100, "reason": "REF:~, GEN:~, 감점:~"},
...
class AIInfluencerWorkflowValidator(WorkflowValidator):
    ...
    ENHANCEMENT_RULES = {
        ...
        "pose_accuracy": (
            "\n\n*** CRITICAL RETRY: Pose mismatch detected! ***\n"
            "You MUST copy the EXACT pose from [POSE REFERENCE] image.\n"
            "Match: framing (full body vs upper body), camera angle, stance,\n"
            "arm positions, leg positions. Do NOT change the framing or angle.\n"
        ),
        "realism": (
    ...
    def validate(
        ...
        **kwargs,
        ...
        if character is not None:
            ...
            pose_images = []
            if reference_images:
                pose_images = reference_images.get("pose", [])
