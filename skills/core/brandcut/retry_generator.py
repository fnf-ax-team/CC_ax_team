...
def generate_with_validation(
    ...
    Args:
        ...
        outfit_spec: OutfitAnalysis 객체 (권장) - 정답 스펙 기반 구조적 검증
            analyze_outfit()에서 반환된 객체를 전달하면:
            - missing_items: 누락 아이템 자동 검출 → 0점
            - mismatched_attributes: 색상/로고 불일치 → 50점 이하
    ...
    for attempt in range(max_retries + 1):
        ...
        try:
            validation_result = validator.validate(
                ...
                outfit_spec=outfit_spec,  # NEW: 정답 스펙 기반 검증
        ...
        history.append(
            {
                ...
                # NEW: 착장 구조적 검증 결과
                "outfit_missing_items": getattr(
                    validation_result, "outfit_missing_items", []
                ),
                "outfit_mismatched_attributes": getattr(
                    validation_result, "outfit_mismatched_attributes", {}
                ),
            }
        )
...
def _build_result(best_image, best_result, history, max_retries) -> dict:
    ...
    return {
        ...
        # NEW: 착장 구조적 검증 결과
        "outfit_missing_items": getattr(best_result, "outfit_missing_items", []),
        "outfit_mismatched_attributes": getattr(
            best_result, "outfit_mismatched_attributes", {}
        ),
    }
