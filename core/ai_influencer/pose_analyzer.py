...
class PoseAnalysisResult:
    ...
    # ★ v3.0 신규 필드 ★
    summary: str = ""  # 한줄요약: 포즈의 전체적 인상을 직관적으로 설명
    tags: List[str] = None  # 태그: 검색용 키워드 리스트
    ...
    def to_preset_format(self) -> Dict[str, Any]:
        """pose_presets.json v3.0 부위별 그룹 형식으로 변환"""
        return {
            "한줄요약": self.summary,
            "태그": self.tags or [],
            "stance": self.stance,
...
★★★ 가시성 규칙 (CRITICAL!) ★★★
- 이미지에서 해당 신체 부위가 **보이지 않으면** 반드시 "보이지않음"으로 기입하세요.
- 절대로 보이지 않는 부위를 추측하거나 지어내지 마세요!
- 프레이밍이 MCU/MS/MFS 등으로 다리가 잘려나간 경우 → 다리 관련 필드 전부 "보이지않음"
- 손이 가려져 안 보이는 경우 → 해당 hand 필드 "보이지않음"
- 팔이 상체 뒤에 숨겨진 경우 → "상체 뒤에 숨겨져 보이지않음"

예시:
- 거울셀피 (상반신만 보임) → left_leg, right_leg, left_foot_*, right_foot_*, left_knee_* 등 전부 "보이지않음"
- 한쪽 팔이 몸 뒤에 숨김 → "상체 뒤로 숨겨져 보이지않음"

## 분석 항목

### 0. summary (한줄요약) ★★★ 최우선! ★★★

포즈의 전체적 인상을 한 문장으로 설명하세요.
텍스트만 읽어도 머릿속에 포즈가 그려질 수 있어야 합니다.
관절 각도 같은 기술적 설명이 아니라, 직관적이고 시각적인 설명을 하세요.

좋은 예시:
- "사이드 앵글에서 기둥에 기대고 한쪽 다리를 뒤로 들어올린 코케티시한 포즈"
- "바닥에 앉아 양무릎을 세우고 두 손으로 턱을 괸 사색적인 포즈"
- "걸어가면서 어깨너머로 카메라를 돌아보는 캔디드 스트릿 무드"
- "거울 앞에서 폰을 들고 턱 아래에 손을 댄 힙한 셀피"

나쁜 예시 (금지):
- "서있는 자세" → 너무 추상적
- "왼팔을 들고 오른다리를 구부린 포즈" → 관절 나열, 인상 없음

### 0-2. tags (태그)

포즈를 검색/분류할 수 있는 키워드 3~7개를 배열로 작성하세요.

카테고리별 태그 예시:
- 자세: "서기", "앉기", "걷기", "기대기", "무릎꿇기"
- 앵글: "정면", "측면", "사이드앵글", "3/4앵글"
- 다리: "다리들기", "다리꼬기", "다리벌림", "한쪽무릎세움"
- 팔: "주머니손", "턱괴기", "머리만지기", "팔짱", "폰들기"
- 무드: "코케티시", "힙한", "시크", "자연스러운", "당당한", "사색적"
- 특징: "S라인", "어깨너머", "뒤돌아보기", "비대칭"
...
[STEP 1] 이미지 하단 경계선 바로 위에 보이는 신체 부위를 확인:
- 발/신발이 보임 → FS
- 종아리/정강이가 보임 (무릎 아래가 잘림) → FS
- 무릎이 보이고 무릎 바로 아래~종아리에서 잘림 → FS
- 허벅지 중간에서 잘림 (무릎이 안 보임) → MFS
- 허리/벨트 부근에서 잘림 → MS
- 가슴/어깨 부근에서 잘림 → MCU
- 얼굴/턱만 보임 → CU
...
- "MS": 머리~허리. 벨트 라인에서 잘림
- "MFS": 머리~허벅지. ★무릎이 안 보임★ 허벅지 중간에서 잘림
- "FS": 머리~무릎 이하. ★무릎이 보임★ (발이 안 보여도 무릎이 보이면 FS)

★★★ 핵심 구분: MFS vs FS ★★★
기준은 "무릎이 보이는가?" 이다:
- 무릎이 보임 (발이 안 보여도) → FS
- 무릎이 안 보임 (허벅지에서 잘림) → MFS

[주의] 다리를 접어 올린 포즈 (4자 다리 등):
- 무릎이 골반/허리 높이까지 올라와서 보이면 → FS
- 발이 안 보여도 무릎 자체가 프레임 안에 있으면 → FS
...
```json
{
    "summary": "바닥에 앉아 양무릎을 세우고 한 손으로 턱을 괸 사색적인 포즈",
    "tags": ["앉기", "약간측면", "턱괴기", "무릎세움", "사색적", "로앵글"],
    "stance": "sit",
