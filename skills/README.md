...
### 🔍 chatbot-dept-mapping
사내 챗봇(Jane_Agent) 로그를 분석하여 사용자 질문을 담당 부서별로 자동 분류하고, 키워드를 매핑합니다.

**주요 기능:**
- 7개 부서 자동 분류 (총무/인사/회계/자금/법무/정보보안·IT/커뮤니케이션)
- 15개 사내 시스템 명칭 정규화 (SA, E-LAW, JIRA 등 오타·동의어 처리)
- 질문 계층 그룹핑 (Topic → Intent, 14개 의도 유형)
- 부서별 TOP 10 FAQ 및 미응답 질문 분석
- 노션 DB 자동 연동 (분석 결과 리포트 생성)
...
**Input:** 챗봇 로그 CSV/Excel (user_query, response, feedback, timestamp)
**Output:** 부서별 분류 테이블 + 노션 분석 리포트
...
### 🧪 chatbot-qa-test
테스트할 질문 목록을 입력하면 챗봇 API를 병렬 호출하고, 결과를 노션 DB에 자동 기록합니다.

**주요 기능:**
- 질문 리스트 일괄 입력 (쉼표 또는 줄바꿈 구분)
- 병렬 API 호출 (최대 5개 동시, 120초 타임아웃)
- 8개 부서 자동 분류 (키워드 매칭)
- 테스트 회차별 타임스탬프 노션 DB 자동 생성
- 결과 상태 관리 (Pass/Fail/검토필요 드롭다운)
...
**Input:** 테스트 질문 리스트 (쉼표 또는 줄바꿈 구분)
**Output:** 노션 DB (질문, 부서, 답변, 결과) + 콘솔 요약 리포트

**필수 환경변수:** `NOTION_API_KEY`, `NOTION_PARENT_PAGE_ID`

---

### 📋 notion-workspace-creator
프로젝트 정보를 입력하면 표준화된 프로젝트 관리 워크스페이스를 노션에 자동 생성합니다.

**주요 기능:**
- 2컬럼 레이아웃 자동 구성 (목차+담당자 / 프로젝트 개요)
- 프로젝트 설명 기반 3대 목표 자동 생성
- 5개 인라인 DB 생성 (예시 데이터 포함): 주요 개발 항목 / WBS / OUTPUT / References / 회의·보고자료
- 프로젝트 유형별 이모지 자동 선택
...
**Input:** 프로젝트명, 설명, PM, 팀원 정보
**Output:** 노션 프로젝트 페이지 (5개 DB + 컬럼 레이아웃 + 예시 데이터)
