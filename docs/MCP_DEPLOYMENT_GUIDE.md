...
| `swap_face` | 얼굴 교체 (포즈/착장/배경 유지) | 원본 이미지, 얼굴 이미지 |
| `generate_ecommerce` | 이커머스 모델 이미지 생성 | 얼굴 이미지, 착장 이미지 |
| `copy_pose` | 포즈 따라하기 (레퍼런스 포즈 복제) | 소스 이미지, 레퍼런스 이미지 |
...
# Storage mode (db/ 폴더 없이 S3에서 프리셋 로드)
FNF_STORAGE_MODE=s3
FNF_S3_BASE_URL=https://tmp-img-s3.s3.ap-northeast-2.amazonaws.com/LINN/fnf-studio
```

> 여러 키를 쉼표로 구분하면 자동 로테이션됩니다 (rate limit 대응).
> `db/` 폴더가 로컬에 있으면 `FNF_STORAGE_MODE=local` (기본값)로 설정합니다.
> `db/` 폴더 없이 S3에서 로드하려면 `FNF_STORAGE_MODE=s3`로 설정합니다.
...
|------|--------|
| **서버 이름** | FNF AI Studio |
| **설명** | F&F 브랜드 AI 이미지 생성 (브랜드컷/인플루언서/배경교체/착장교체/얼굴교체/이커머스/포즈따라하기) |
| **카테고리** | 이미지 생성 |
| **공개 범위** | Private (승인 필요) |
...
| swap_face | 얼굴 교체 (포즈/착장/배경 유지) |
| generate_ecommerce | 이커머스 모델 이미지 생성 |
| copy_pose | 포즈 따라하기 (레퍼런스 포즈 복제) |
| list_options | 비율/해상도/비용 조회 |
| list_presets | 프리셋/캐릭터 목록 조회 |

### 필요 환경변수
- GEMINI_API_KEY: Gemini API 키 (쉼표 구분 복수 키 지원)
- FNF_STORAGE_MODE: `local` (기본) 또는 `s3` (db/ 폴더 없이 S3 사용)
- FNF_S3_BASE_URL: S3 base URL (s3 모드일 때 필요)
